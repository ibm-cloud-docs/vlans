...
Servers can establish trunks with Automatic and Premium VLANs so that traffic can flow between devices by using different VLANs than the ones chosen during provisioning.
{: shortdesc}
...
Network component groups are multiple physical interfaces that participate in a logical redundant connection. A device can have multiple network interfaces and network interface groups. When you establish a trunk to any member in the group, that VLAN trunk is replicated to all group members.

## Managing VLAN trunks using the UI
{: #ui-manage-trunks}
{: ui}

You can establish or remove VLAN trunks by using the UI.
...
Create a VLAN trunk in the console by taking the following steps:
...
1. Select a VLAN by clicking its fully qualified name in the list of VLANs.
1. Click **Attach trunk** in the **Devices with trunks** section.
1. Select a resource from the devices list in the window that appears.
1. Click **Attach** to establish the trunk, or click **Cancel** to close the window without adding the trunk.
...
Manually configure your host's network with your added VLAN trunks before they can be used to pass traffic.
{: note}
...
1. Click **Remove trunk** to remove the trunk, or click **Cancel** to close the window without removing the trunk.

## Managing VLAN trunks with the API
{: #api-manage-trunks}
{: api}

You can create and remove VLAN trunks with the API.

### Creating VLAN trunks with the API
{: #api-create-vlan-trunks}

For more information on creating VLAN trunks with the API, see [addNetworkVlanTrunks](https://sldn.softlayer.com/reference/services/SoftLayer_Network_Component/addNetworkVlanTrunks/){: external}

### Removing VLAN trunks with the API
{: #api-remove-vlan-trunks}

For more information on removing VLAN trunks with the API, see [clearNetworkVlanTrunks](https://sldn.softlayer.com/reference/services/SoftLayer_Network_Component/clearNetworkVlanTrunks/){: external}
