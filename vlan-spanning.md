...
Enable VLAN spanning only if you can't or don't want to enable Virtual Routing and Forwarding (VRF). For more information, see [Network segmentation](/docs/containers?topic=containers-subnets#basics_segmentation).
{: important}
...
When VLAN spanning is enabled, it affects the entire account. You cannot exempt any subnets, VLANs, or devices from VLAN spanning. If you currently rely solely on private subnets to separate servers into roles or tiers, enabling VLAN spanning removes that separation.
{: note}
...
This option does not appear for accounts that are incompatible with VLAN Spanning.
{: note}
...
1. Select **VLAN Spanning** to enable or disable VLAN spanning as needed.

   Toggling VLAN Spanning within a short amount of time might delay the application.
   {: note}
