...
Any device that has a network connection is associated with a VLAN. Dedicated servers have both a public and private network connection, so you see those devices that are associated with both public and private VLANs.
...
If each server is on a different subnet, then by default, they are not able to communicate by using IP addresses. Technically, your servers can communicate by using OSI Model Layer 2 methods because they are on the same VLAN (a Layer 2 construct). For Internet Protocol (IP) (also called Layer 3) communication to work, you can do either of the following tasks:
...
Keep in mind that [VLAN Spanning](/docs/vlans?topic=vlans-vlan-spanning) has implications, so review the feature in detail before you enable it.
